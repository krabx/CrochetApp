...
final class SchemaViewController: UIViewController {
    ...
    private var elementsData: [String] = []
    ...
    override func viewDidLoad() {
        ...
        let deleteAction = UIAction(title: "Режим удаления", image: UIImage(systemName: "trash")) { [unowned self] _ in
            currentMode = .delete
            setDashedBorderForAllSubView(in: schemaImageView)
        }
        ...
        // MARK: - Adding tap to schemaImageView

        let tap = UITapGestureRecognizer(
            target: self,
            action: currentMode == .insert ? #selector(touchedScreen(touch:)) : nil
        )
...
extension SchemaViewController {
    ...
    private func setDashedBorderForAllSubView(in imageView: UIImageView) {
        for subView in imageView.subviews {
            ...
            dashedBorder.cornerRadius = 10.0
...
// MARK: - Extension for ElementCollectionViewControllerDelegate
